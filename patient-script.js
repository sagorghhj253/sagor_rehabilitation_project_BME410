...
function setupVideoControls() {
    ...
    playPauseBtn.addEventListener('click', () => {
        if (videoElement.paused) {
            videoElement.play();
            playPauseBtn.innerHTML = '<span>⏸</span> Pause Video';
            playPauseBtn.classList.add('pause');
        } else {
            videoElement.pause();
            playPauseBtn.innerHTML = '<span>▶</span> Play Video';
    ...
    videoElement.addEventListener('ended', () => {
        playPauseBtn.innerHTML = '<span>▶</span> Play Again';
        playPauseBtn.classList.remove('pause');
    });
}
...
function showSessionCompleteMessage(sessionData) {
    ...
    notification.innerHTML = `
        <div class="notification-content">
            <h4>✅ Exercise Session Completed!</h4>
            <p><strong>${sessionData.exercise}</strong></p>
            <p>Reps: ${sessionData.reps} | Duration: ${sessionData.duration} min</p>
            <p>Form Score: ${sessionData.accuracy}%</p>
            <p><em>${sessionData.feedback}</em></p>
            <button onclick="viewMyProgress()" class="view-progress-btn">
                📊 View Progress
            </button>
        </div>
    `;
...
// Function to add progress button to navigation
